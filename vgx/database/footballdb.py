...
async def get_group_settings(chat_id: int) -> dict:
    default_settings = {
        "chat_id": chat_id,
        "auto_delete": 0,         # 0 (off), 30, 300, 400, 2400
        "pin_messages": False,
        "live_schedule": 0,       # 0 (off), 60, 300, 1200, 1800, 3600
        "last_live_run": datetime.utcnow() - timedelta(days=1),
        "modules": {
            "live": True, "standings": True, "topscorers": True
        }
    }
...
# --- TARGET MANAGEMENT (PM Control) ---
async def set_user_target(user_id: int, target_chat_id: int):
    await targets_col.update_one(
        {"user_id": user_id}, {"$set": {"active_target": target_chat_id}}, upsert=True
    )
...
async def set_cached_api(cache_key: str, data: dict, ttl_seconds: int = 1800):
    ...
    await cache_col.update_one(
        {"_id": cache_key}, {"$set": {"data": data, "expires_at": expires_at}}, upsert=True
    )
