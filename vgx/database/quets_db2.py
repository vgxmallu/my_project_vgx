...
async def get_chat(chat_id: int) -> dict:
    """Fetch settings for a chat, or return safe defaults."""
    chat = await chats_col.find_one({"chat_id": chat_id})
    if not chat:
        return {
            "chat_id": chat_id,
            "enabled": False,
            "interval": 60,      # Default interval: 60 minutes
            "delete_after": 0,   # Default auto-delete: 0 (Off)
            "pin": False,        # Default pin: Off
            "last_msg_id": None, 
            "last_sent": 0
        }
    return chat
...
async def update_chat(chat_id: int, **kwargs):
    """Update specific settings for a chat seamlessly."""
    await chats_col.update_one({"chat_id": chat_id}, {"$set": kwargs}, upsert=True)

async def get_due_chats() -> list:
    """Finds all active chats where the requested time interval has passed."""
    now = int(time.time())
