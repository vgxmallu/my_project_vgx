...
async def get_config():
    """Fetches global bot settings from MongoDB."""
    config = await settings_col.find_one({"_id": "global_config"})
    if not config:
        default_config = {"_id": "global_config", "interval": 3600, "auto_delete": 300}
        await settings_col.insert_one(default_config)
        return default_config
    return config

async def update_config(key: str, value: int):
    """Updates a global setting in MongoDB."""
...
@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("addgroup"))
async def add_group(client: Client, message: Message):
    try:
        group_id = int(message.command[1])
        # Add to DB, enabling by default and setting pin to false
        await groups_col.update_one(
            {"_id": group_id},
            {"$set": {"enabled": True}, "$setOnInsert": {"pin": False}},
            upsert=True
        )
        await message.reply_text(f"✅ Group `{group_id}` added to target list.")
    except (IndexError, ValueError):
        await message.reply_text("**Usage:** `/addgroup <group_id>`")

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("removegroup"))
async def remove_group(client: Client, message: Message):
    try:
        group_id = int(message.command[1])
        await groups_col.delete_one({"_id": group_id})
        await message.reply_text(f"✅ Group `{group_id}` removed from target list.")
    except (IndexError, ValueError):
        await message.reply_text("**Usage:** `/removegroup <group_id>`")

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("enable"))
async def enable_group(client: Client, message: Message):
    try:
        group_id = int(message.command[1])
        result = await groups_col.update_one({"_id": group_id}, {"$set": {"enabled": True}})
        if result.matched_count:
            await message.reply_text(f"✅ Module **ENABLED** for group `{group_id}`.")
        else:
            await message.reply_text("❌ Group not found in database. Use `/addgroup` first.")
    except (IndexError, ValueError):
        await message.reply_text("**Usage:** `/enable <group_id>`")

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("disable"))
async def disable_group(client: Client, message: Message):
    try:
        group_id = int(message.command[1])
        result = await groups_col.update_one({"_id": group_id}, {"$set": {"enabled": False}})
        if result.matched_count:
            await message.reply_text(f"🚫 Module **DISABLED** for group `{group_id}`.")
        else:
            await message.reply_text("❌ Group not found in database.")
    except (IndexError, ValueError):
        await message.reply_text("**Usage:** `/disable <group_id>`")

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("setpin"))
async def set_pin(client: Client, message: Message):
    try:
        group_id = int(message.command[1])
        ...
        is_pinned = (status == "on")
        result = await groups_col.update_one({"_id": group_id}, {"$set": {"pin": is_pinned}})
        if result.matched_count:
            await message.reply_text(f"📌 Pin messages set to **{status.upper()}** for group `{group_id}`.")
        else:
            await message.reply_text("❌ Group not found in database.")
    except (IndexError, ValueError):
        await message.reply_text("**Usage:** `/setpin <group_id> <on/off>`")

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("setinterval"))
async def set_interval(client: Client, message: Message):
    try:
        seconds = int(message.command[1])
        await update_config("interval", seconds)
        await message.reply_text(f"⏱ Schedule interval updated to `{seconds}` seconds in Database.")
    except (IndexError, ValueError):
        await message.reply_text(
            "**Usage:** `/setinterval <seconds>`\n\n"
            "**Examples:** 1m = `60` | 5m = `300` | 20m = `1200` | 30m = `1800` | 1h = `3600`"
        )

@app.on_message(filters.private & filters.user(Config.ADMIN_ID) & filters.command("setautodelete"))
async def set_autodelete(client: Client, message: Message):
    try:
        seconds = int(message.command[1])
        await update_config("auto_delete", seconds)
        await message.reply_text(f"🗑 Auto-delete time updated to `{seconds}` seconds in Database.")
    except (IndexError, ValueError):
        await message.reply_text(
            "**Usage:** `/setautodelete <seconds>`\n\n"
            "**Examples:** 30s = `30` | 300s = `300` | 400s = `400` | 2400s = `2400`"
        )
...
async def fetch_todays_matches():
    """Fetches today's football matches using TheSportsDB free V1 API."""
    today = datetime.utcnow().strftime('%Y-%m-%d')
    url = f"https://www.thesportsdb.com/api/v1/json/123/eventsday.php?d={today}&s=Soccer"
        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    events = data.get("events")
                    if not events:
                        return "No football matches scheduled for today."
                    
                    text = f"⚽️ **Today's Football Matches** ({today})\n\n"
                    for event in events[:15]:  # Limit to 15 to avoid massive message blocks
                        home = event.get("strHomeTeam", "Unknown")
                        away = event.get("strAwayTeam", "Unknown")
                        time = event.get("strTime", "TBA")
...
async def auto_delete_task(chat_id: int, message_id: int, delay: int):
    """Deletes a specific message after a delay."""
    await asyncio.sleep(delay)
    try:
        await app.delete_messages(chat_id, message_id)
    except Exception:
        pass # Ignore errors if bot lacks delete rights or message is already gone

async def scheduler_loop():
    """Main loop checking MongoDB settings and posting to targets."""
    # Ensure config exists before looping
    await get_config()
    
    while True:
        config = await get_config()
        interval = config.get("interval", 3600)
        auto_del_time = config.get("auto_delete", 300)
        
        # Get all groups where 'enabled' is True
        cursor = groups_col.find({"enabled": True})
        active_groups = await cursor.to_list(length=None)
        
        if active_groups:
            match_text = await fetch_todays_matches()
            
            if match_text:
                for grp in active_groups:
                    group_id = grp["_id"]
                    should_pin = grp.get("pin", False)
                    
                    try:
                        # 1. Send the schedule
                        msg = await app.send_message(chat_id=group_id, text=match_text)
                        
                        # 2. Pin if DB says True
                        if should_pin:
                            try:
                        ...
                        if auto_del_time > 0:
                            asyncio.create_task(
                                auto_delete_task(group_id, msg.id, auto_del_time)
                            )
                            
                    except Exception as e:
                        print(f"Failed to post in {group_id}: {e}")
        
        # Sleep for the configured custom interval
