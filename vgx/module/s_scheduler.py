...
async def send_managed_message(client: Client, chat_id: int, text: str, reply_markup=None):
    ...
    msg = await client.send_message(chat_id=chat_id, text=text, reply_markup=reply_markup, disable_web_page_preview=False)
    ...
    while True:
        try:
            ...
            async for group in db.group_settings.find({"live_schedule": {"$gt": 0}}):
                ...
                sched_time = group["live_schedule"]
                last_run = group.get("last_live_run", now - timedelta(days=1))

                if (now - last_run).total_seconds() >= sched_time:
                    # Defaulting to football for scheduled pings, but checks if enabled
                    if group.get("modules", {}).get("football", True):
                        live_text = await format_live_scores("football")
                        await send_managed_message(client, chat_id, f"📡 **SCHEDULED UPDATE**\n\n{live_text}")
